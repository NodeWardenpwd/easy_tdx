...
| 页面 | 功能 |
|------|------|
| **单标的回测** | 选标的取行情，挑 18 个内置策略之一，调参数，K 线主图标买卖点，净值回撤双轴图，19 项绩效指标，成交明细 |
| **组合回测** | 多只股票等权组合，组合净值曲线（按日期对齐求和），各标的净值叠加对比，横向绩效表 |
| **参数寻优** | 勾选 1-2 个参数，填取值列表，网格遍历所有组合，排名表 + 二维热力图，最优点一键跳转回测 |
| **结果对比** | 选 2-4 个已完成回测，归一化净值叠加，8 项指标横向 PK |

取行情支持日期范围选择，超过 800 根自动翻页拼接。技术栈：Vue 3 + Vite + TypeScript + Pinia + ECharts（按需引入）。
